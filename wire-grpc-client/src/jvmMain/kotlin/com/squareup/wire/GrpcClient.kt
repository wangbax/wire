...
inline fun <reified T : Service> GrpcClient.create(): T = create(T::class)

/** Returns a [service] that makes gRPC calls using this client. */
fun <T : Service> GrpcClient.create(service: KClass<T>): T {
...
fun GrpcClient.newBuilder(): GrpcClientBuilder {
...
fun GrpcClient.Companion.Builder(): GrpcClientBuilder = GrpcClientBuilder()

internal fun GrpcClient.newCall(
  method: GrpcMethod<*, *>,
...
class GrpcClientBuilder {
  ...
  fun client(client: OkHttpClient): GrpcClientBuilder {
  ...
  fun callFactory(client: Call.Factory): GrpcClientBuilder = apply {
    this.client = client
  }

  fun baseUrl(baseUrl: String): GrpcClientBuilder = apply {
    this.baseUrl = baseUrl.toHttpUrl()
  }

  fun baseUrl(url: GrpcHttpUrl): GrpcClientBuilder = apply {
    this.baseUrl = url
  }
